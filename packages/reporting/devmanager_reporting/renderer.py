    ...
    *,
    ...
    if score is not None:
        ...
        for d in (score.deductions or []):
            ...
            sev_counts[sev] += 1
            sev_deductions[sev] = sev_deductions.get(sev, 0.0) + d.get("capped_deduction", d.get("actual_deduction", 0.0))
        ...
        if sev_counts:
            ...
            for sev in _SEVERITY_ORDER:
                if sev in sev_counts:
                    ...
                    lines.append(
                        f"| {emoji} {sev.capitalize()} | {sev_counts[sev]} | {sev_deductions.get(sev, 0.0):.2f} |"
                    )
            lines.append("")
    ...
    if not findings:
        ...
        sorted_findings = sorted(
            ...
            key=lambda f: (_SEVERITY_ORDER.index(
                getattr(f, "severity", "informational").lower()
            ) if getattr(f, "severity", "informational").lower() in _SEVERITY_ORDER else 99,
            getattr(f, "category", ""),
            ),
        )
    ...
    lines += [
        f"*Report generated by DevManager v{_APP_VERSION} (shadow/read-only mode) — {generated_at}*",
    ]
